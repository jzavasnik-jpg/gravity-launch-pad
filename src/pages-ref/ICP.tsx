...
const ICP = ({ hideNavigation = false }: ICPProps) => {
  ...
  useEffect(() => {
    ...
    if (isCompletingInterview) {
      timeout = setTimeout(() => {
        console.warn('⚠️ Safety timeout triggered: Resetting completion state');
  ...
  }, [isCompletingInterview]); // Removed toast from dependencies
  ...
  const handleContinue = useCallback(async () => {
    ...
    if (isCompletingInterview) {
      console.log('⚠️ Interview completion already in progress, ignoring duplicate request');
      return;
    }
    ...
    if (isLastQuestion) {
      ...
      if (!sessionId) {
        console.warn('⚠️ No session ID found, attempting to initialize now...');
      ...
      if (sessionId) {
        try {
          ...
          try {
            console.log('⚠️ Attempting fallback: Creating new session...');
            if (appState.userId && appState.userName) {
              const newSession = await createICPSession(
                appState.userId,
                appState.userName,
        ...
        console.warn('⚠️ No session ID found even after retry, cannot persist to database');
        setIsCompletingInterview(false);
  ...
  }, [currentAnswer, currentQ, isLastQuestion, isCompletingInterview, updateICPAnswer, setCompleted, setCurrentQuestion, navigate, appState.sessionId, appState.userId, appState.userName, appState.selectedCoreDesire, appState.selectedSixS, appState.gravityICP.answers, initializeSession, toast]);
