...
export default function LandingPadGeneratePage() {
    ...
    // Load setup data from sessionStorage (set by the setup page)
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const storedData = sessionStorage.getItem('landingPadSetupData');
            if (storedData) {
                try {
    ...
    // Start generation when setup data is loaded
    useEffect(() => {
        if (setupData) {
            generateLandingPage();
        }
    ...
    const generateLandingPage = async () => {
        ...
        if (typeof window !== 'undefined') {
            sessionStorage.removeItem('landingPadSetupData');
        }

        // Auto-navigate after a brief pause
        setTimeout(() => {
            router.push('/landing-pad/editor');
        }, 1500);
    };
