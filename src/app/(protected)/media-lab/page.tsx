...
export default function MediaLabPage() {
    ...
    // Load data from sessionStorage (replaces React Router's location.state)
    useEffect(() => {
        const stored = sessionStorage.getItem('mediaLabProjectData');
        if (stored) {
            try {
                ...
                // Clear after loading
                sessionStorage.removeItem('mediaLabProjectData');
            } catch (error) {
                console.error('Error parsing media lab project data:', error);
            }
        }
    }, []);
