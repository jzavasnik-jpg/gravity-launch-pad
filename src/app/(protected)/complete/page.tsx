...
export default function CompletePage() {
  ...
  }, [appState.gravityICP.completed, router]);
  ...
  useEffect(() => {
    async function generateContent() {
      ...
      if (!sessionId && appState.userId) {
        ...
        try {
          const { getLatestICPSession } = await import("@/lib/database-service");
        ...
        try {
          ...
          if (marketing && generatedAvatars[0]?.id) {
            ...
            if (savedMarketing) {
              ...
              console.warn('Failed to save marketing statements to Firebase');
